...
function createGridGeometry(): THREE.BufferGeometry {
  ...
  const xLines = Math.floor(
    GRID_WIDTH / GRID_SPACING,
  );

  const zLines = Math.floor(
    GRID_DEPTH / GRID_SPACING,
  );

  const xSegments = Math.ceil(
    GRID_WIDTH / VERTEX_STEP,
  );

  const zSegments = Math.ceil(
    GRID_DEPTH / VERTEX_STEP,
  );
  ...
  for (
    ...
    for (
      ...
      const xStart =
        -GRID_WIDTH / 2 +
        (segment / xSegments) * GRID_WIDTH;

      const xEnd =
        -GRID_WIDTH / 2 +
        ((segment + 1) / xSegments) * GRID_WIDTH;
  ...
  for (
    ...
    for (
      ...
      const zStart =
        -GRID_DEPTH / 2 +
        (segment / zSegments) * GRID_DEPTH;

      const zEnd =
        -GRID_DEPTH / 2 +
        ((segment + 1) / zSegments) * GRID_DEPTH;
...
function applyPalette(): void {
  ...
  material.uniforms.uWaveColor?.value.copy(
    palette.waveColor,
  );
...
function updateTrailTexture(
  ...
  if (material.uniforms.uTrailCount) {
    material.uniforms.uTrailCount.value =
      trail.length;
  }
}
...
function addTrailPoint(
  ...
  const elapsed = lastPoint
    ? Math.max(
        now - lastPoint.createdAt,
        16,
      )
    : 16;
...
function initialize(): void {
  ...
  try {
    ...
    trailTexture.minFilter =
      THREE.NearestFilter;

    trailTexture.magFilter =
      THREE.NearestFilter;

    trailTexture.generateMipmaps =
      false;
