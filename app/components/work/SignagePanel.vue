...
<template>
  ...
  >
    <div class="display-flow grid justify-items-center p-5 sm:p-6 md:p-8 xl:p-10">
      <div :class="['flow-node w-[min(100%,15rem)]', nodeClasses]">
        <small class="font-mono text-[0.54rem] leading-6 text-muted">{{ content.remote.label }}</small>
      ...
      <div
        class="player-node grid w-full grid-cols-1 items-center gap-[0.8rem] border border-line-strong p-[0.8rem] sm:grid-cols-[1fr_auto_1fr]"
      >
        <template
        ...
        >
          ...
          <div :class="nodeClasses">
            <small class="font-mono text-[0.54rem] leading-6 text-muted">{{ node.label }}</small>
            <strong class="font-display text-base uppercase">{{ node.title }}</strong>
          </div>
        </template>
      </div>

      <div class="flow-split mt-4 grid w-full grid-cols-1 gap-4 sm:grid-cols-2">
        <span
        ...
        >
          ...
          <small class="font-mono text-[0.54rem] leading-6 text-muted">{{ output.description }}</small>
        </span>
      </div>
    </div>
  </SharedPanelFrame>
